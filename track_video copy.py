...
while True:
    ...
    if len(tracks) > 0:
        ...
        img = draw_boxes(img, [t.tlbr for t in confirmeds], thickness=2)
        img = draw_track_labels(img, confirmeds, thickness=1)

        tentatives = [t for t in tracks if t.detector_confidence == 0 or t.tentative]
        img = draw_boxes(img, [t.tlbr for t in tentatives], color=(255,255,255), thickness=1)
#        tracked_img = draw_track_labels(tracked_img, tentatives)
        fps_msg = 'FPS: %.1f' % (fps)
        img = cv2.putText(img, fps_msg, (5, 20), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                            (0,0,255), 2)
